...
const createTenantClient = async (
  tenantId: number,
  email: string,
  apiKey: string,
  baseUrl = getBaseUrl(),
...
describe("Auth compatibility endpoints", () => {
  it("POST /api/v1/jwt/fetch_api_key should return the existing API key and optional profile", async () => {
    ...
    expect((res.body.user as Record<string, unknown>).email).to.equal(
      seeded.email,
    );
    expect(
      (res.body.user as Record<string, unknown>).is_imported_stub,
    ).to.equal(false);
  });
  ...
  it("POST /api/v1/jwt/fetch_api_key should reject JWTs missing the email claim", async () => {
    ...
    expect(res.status).to.equal(400);
    expect(res.body.msg).to.equal(
      "No email specified in JSON web token claims",
    );
  });
  ...
  it("POST /api/v1/users/me/api_key/regenerate should regenerate the authenticated user's API key and invalidate the old key", async () => {
    ...
    const freshClient = await createTenantClient(
      tenantId,
      seeded.email,
      regeneratedKey,
    );
  ...
  it("GET /api/v1/dev_list_users should list active non-bot users across tenants with realm URLs", async () => {
    ...
    const secondOwner = await seedUser(db, tenantA, {
      role: 200,
      email: "zeta-admin@test.local",
    });
    const member = await seedUser(db, tenantB, { role: 400 });
    const secondMember = await seedUser(db, tenantB, {
    ...
    const inactive = await seedUser(db, tenantB, {
    ...
    const alphaRealmUrl =
      port === ""
        ? "http://alpha.test.local"
        : `http://alpha.test.local:${port}`;
    const betaRealmUrl =
      port === "" ? "http://beta.test.local" : `http://beta.test.local:${port}`;
    ...
    const directAdmins = res.body.direct_admins as Array<
    ...
    expect(directAdmins.some((entry) => entry.email === bot.email)).to.equal(
      false,
    );
    expect(directUsers.some((entry) => entry.email === bot.email)).to.equal(
      false,
    );
    expect(
      directAdmins.some((entry) => entry.email === inactive.email),
    ).to.equal(false);
    expect(
      directUsers.some((entry) => entry.email === inactive.email),
    ).to.equal(false);
  });
