...
describe("PATCH /api/v1/realm/user_settings_defaults", () => {
  it("should update realm user setting defaults with Zulip-compatible validation", async () => {
    ...
    expect(res.body.ignored_parameters_unsupported).to.deep.equal([
      "emoji_set",
    ]);

    const row = await db("tenant_user_setting_default")
  ...
  it("should reject non-admin and invalid realm user setting default updates", async () => {
    ...
    const batchingRes = await admin.client.patch(
      "/realm/user_settings_defaults",
      {
    ...
    expect(batchingRes.body.msg).to.equal(
      "Invalid email batching period: -1 seconds",
    );

    const emojisetRes = await admin.client.patch(
      "/realm/user_settings_defaults",
      {
        emojiset: "invalid",
      },
    ...
    expect(emojisetRes.body.msg).to.equal(
