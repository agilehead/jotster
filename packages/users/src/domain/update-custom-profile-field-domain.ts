...
import type { CustomProfileFieldUpdates } from "../repo/update-custom-profile-field.ts";
import { mapCustomProfileFieldToCompatRecord } from "./map-custom-profile-field-to-compat-record.ts";

type UpdateFieldInput = CustomProfileFieldUpdates;
...
export const updateCustomProfileFieldDomain = async (
  ...
  const existingFields = await getCustomProfileFields(
    options,
    actingUser.tenantId,
  );
  ...
  const nextName =
    input.name !== undefined ? input.name.trim() : currentField.Name;
  const nextFieldType =
    input.fieldType !== undefined ? input.fieldType : currentField.FieldType;
  const nextDisplayInProfileSummary =
    input.displayInProfileSummary !== undefined
      ? input.displayInProfileSummary
      : currentField.DisplayInProfileSummary;
  const nextUseForUserMatching =
    input.useForUserMatching !== undefined
      ? input.useForUserMatching
      : currentField.UseForUserMatching;

  for (let i = 0; i < existingFields.length; i++) {
    if (
      existingFields[i].Id !== fieldId &&
      existingFields[i].Name === nextName
    ) {
  ...
  if (
    nextDisplayInProfileSummary === (1 as int) &&
    nextFieldType === (6 as int)
  ) {
    return err("Field type not supported for display in profile summary.");
  }

  if (
    nextUseForUserMatching === (1 as int) &&
    nextFieldType !== (1 as int) &&
    nextFieldType !== (7 as int)
  ) {
  ...
  const updated = await updateCustomProfileField(
    options,
    actingUser.tenantId,
    fieldId,
    {
