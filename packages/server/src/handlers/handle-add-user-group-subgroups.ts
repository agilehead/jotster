...
import { addUserGroupSubgroupsDomain } from "@jotster/permissions/Jotster.Permissions.js";
import { parseId } from "@jotster/core/Jotster.Core.js";
import { toLong, toLongArray } from "../helpers/body.ts";
import type { AppContext } from "../helpers/app-context.ts";

export const handleAddUserGroupSubgroups = async (
  req: Request,
  res: Response,
  app: AppContext
): Promise<void> => {
  ...
  const result = await addUserGroupSubgroupsDomain(app.options, user, toLong(groupId), addIds);
  if (!result.success) {
