  ...
  Result<
    {
      ...
      alreadySubscribed: Record<string, string[]>;
    },
    string
  >
> => {
  ...
  try {
    ...
    if (
      ...
      channel.HistoryPublicToSubscribers =
        (createParams.historyPublicToSubscribers ?? 1) as int;
    ...
    if (principals !== undefined && principals.length > 0) {
      ...
      if (
        ...
        return err(
          "Only admins or channel creator can subscribe others to private channels",
        );
      }
      ...
      for (let i = 0; i < principals.length; i++) {
        ...
        // Look up by email
        const targetUser = await db0.Users.Where(
          (u) => u.TenantId === tenantId0,
        )
          .Where((u) => u.Email === principal0)
          .FirstOrDefaultAsync();
    ...
    for (let i = 0; i < targetUsers.Count; i++) {
      ...
      if (existingSub !== undefined) {
        ...
        const count = await countUserSubscriptions(
          options,
          user.tenantId,
          target.id,
        );
